...
- **Standalone AI Gateway**: Operating as a dedicated microservice (similar in topology to Kong), this is the primary ingress point for AI workloads. Before any traffic hits the Temporal orchestrator, this gateway performs **Token Validation** (OAuth2/JWT) and enforces **SCA Consent Management**, ensuring the calling Agent or user is authorized to perform the requested banking intent.
- **AI Security (Pluggable Guardrails)**: Before ANY user payload enters the foundational models, a brand-agnostic Guardrail Interceptor inspects the vectors and chat history to block Prompt Injections and **Context Poisoning**. This architecture allows for swapping between internal Security SLMs, Nvidia NeMo Guardrails, or Lakera without code rewrites.
